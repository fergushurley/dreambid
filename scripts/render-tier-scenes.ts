...
async function main(){
 loadEnvConfig(process.cwd());const {renderProject}=await import("../lib/render");const rows=[];
 for(const concept of featuredConcepts(canonicalBrief).concepts){
  ...
  rows.push({variant:concept.id,imageUrl,geometryKey:sceneGeometryKey(project),renderer:"Blender Cycles",quality:"max",residence:canonicalSiteContext.residence,durationSeconds:Math.round((Date.now()-start)/1000),source:"Deterministic render of this exact ProjectSpec; fixture geometry, illustrative finishes."});
