...
import { canonicalSiteContext, DEMO_RESIDENCE_VERSION } from "../fixtures/site-context";
...
async function main(){
 ...
 for(const concept of featuredConcepts(canonicalBrief).concepts){
  const p=fixtureProject(canonicalBrief,concept,canonicalSiteContext),imageUrl=`/demo/photoreal/${images[concept.palette]}-two-story.png`;
  ...
  const row={residenceVersion:DEMO_RESIDENCE_VERSION,variant:concept.id,imageUrl,geometry,rotations:p.elements.map(e=>[e.id,e.rotationDeg??0]),model:"Built-in image generation",generatedAt:"2026-09-10",source:"Retained AI concept reference for curated feature scope; geometry is approximate, not a property photograph. See docs/CONCEPT_TIERS.md for prompts."};
