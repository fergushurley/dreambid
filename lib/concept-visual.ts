...
export async function generateConceptVisual(project:ProjectSpec,site:SiteContext,photos:ProjectBrief["photos"],demoMode:boolean):Promise<VisualResult>{
 const referenceQuality="max" as const;
 const signature=visualSignature(project),model=process.env.OPENAI_IMAGE_MODEL||"gpt-image-2";
 const hash=createHash("sha256").update(JSON.stringify({version:1,rendererVersion:RENDERER_VERSION,referenceQuality,signature,site,photos,model})).digest("hex").slice(0,24);
 ...
 const task=(async():Promise<VisualResult>=>{
   ...
   const scene=await renderProject(project,referenceQuality);
   ...
   try {
     const referenceBytes=await readFile(path.join(folder,`${sceneHash(project,referenceQuality)}.png`));
