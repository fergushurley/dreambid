...
export async function renderProject(project: ProjectSpec, quality: "preview" | "max" = "preview"): Promise<ProjectSpec["scene"]> {
  const hash = sceneHash(project, quality);
  const existing = inFlight.get(hash); if (existing) return existing;
  const task = renderOnce(project, hash, quality).finally(() => inFlight.delete(hash));
  inFlight.set(hash, task); return task;
}
async function renderOnce(project: ProjectSpec, hash: string, quality: "preview" | "max"): Promise<ProjectSpec["scene"]> {
  ...
  if (blender) {
    try {
      await writeFile(input, JSON.stringify({ ...project, siteContext: siteForAddress(project.propertyAddress, project.siteContextId === "site-maple-demo") }));
      await new Promise<void>((resolve, reject) => {
        const child = spawn(blender, ["--background", "--python-exit-code", "1", "--python", path.join(process.cwd(), "blender", "render.py"), "--", input, output, quality], { stdio: "ignore", shell: false });
        const timeout = setTimeout(() => { child.kill("SIGKILL"); reject(new Error("Blender render timed out.")); }, quality === "max" ? 300000 : Math.min(90000, Number(process.env.BLENDER_TIMEOUT_MS) || 60000));
