...
import { ArrowLeft, ArrowRight, Check, ChevronDown, LockKeyhole, Minus, MousePointer2, Plus, RotateCw, Search, ShieldCheck, Trash2 } from "lucide-react";
...
import { applyLayoutPatch, budgetSummary, dimensionBounds, suggestedPosition } from "@/lib/layout";
...
export function LayoutEditor({ project, site, demoMode, onAssistantApply, onChange, onClose }: { demoMode: boolean; onAssistantApply: (project: ProjectSpec, meta: EngineMeta) => void; project: ProjectSpec; site: SiteContext; onChange: (project: ProjectSpec, committed: boolean) => void; onClose: () => void }) {
  ...
  const [materialView, setMaterialView] = useState(true);
  ...
  function rotate(e: ProjectElement) { commit({summary:`Rotate ${e.label} 90° clockwise`,operations:[{action:"rotate",elementId:e.id}]}); }
  const selectedBounds = selected ? dimensionBounds(selected) : null;
  ...
  function move(event: PointerEvent<SVGSVGElement>) {
    ...
    const bounds = dimensionBounds(current.element);
    const e=current.element;
    const width=snap(Math.max(bounds.min.widthFt??1,Math.min(bounds.max.widthFt??100,e.size.widthFt+(current.mode==="resize-right"?dx:current.mode==="resize-left"?-dx:0))));
    const depth=snap(Math.max(bounds.min.depthFt??1,Math.min(bounds.max.depthFt??100,e.size.depthFt+(current.mode==="resize-rear"?dy:current.mode==="resize-front"?-dy:0))));
  ...
  return <section className="layout-workspace" aria-label="Customize layout">
    <div className="editor-topline"><button className="text-button" onClick={onClose}><ArrowLeft size={15}/> Preview your design</button><span>PROJECT REVISION {project.version} · FEET</span><button className="button primary" onClick={onClose}>Use this layout <Check size={15}/></button></div>
