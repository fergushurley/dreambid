...
import { ConceptThumbnail } from "./concept-thumbnail";
...
export function DreamBid() {
  ...
  useEffect(() => {
    if (!project) return;
    try { localStorage.setItem("dreambid-project-v1", JSON.stringify({ project, site: siteForStorage(site), brief: { ...brief, photos: [] }, meta })); } catch { /* The active in-memory project remains usable. */ }
  ...
  async function lookupProperty() {
    const address=brief.propertyAddress;
    await run("Finding your property context…",async()=>{
      const result=await post<{site:SiteContext}>("/api/property-context",{address,useDemo:brief.useDemoSite,demoMode},45000);
      setSite(result.site);
    });
  }
  ...
  async function selectConcept(concept: RenovationConcept) {
    await run("Turning your concept into a scoped project…", async () => {
      ...
      setProject(checkFeasibility(result.data,result.site,false)); setSite(result.site); setMeta(result.meta); setStage("editor"); setView("concept"); setQuotes([]); setRecommendation(null);
    });
  }
  async function revise() {
    ...
    await run("Revising the scope and checking what stays…", async () => {
      ...
      setProject(result.data); setSite(current => current.propertyAddress === result.site.propertyAddress ? {...result.site, propertyContext: current.propertyContext} : result.site); setMeta(result.meta); setRevision(""); setQuotes([]); setRecommendation(null); setView("concept");
      if (result.site.isDemo) void renderProject(result.data);
    });
  }
  async function regenerateAppearance(selected:ProjectSpec|null=project) {
    if (!selected) return;
    const original = selected, signature = visualSignature(selected);
    await run("Creating your AI concept from the current layout…", async () => {
      setVisualNote("");
      if(site.isDemo && !matchingScene(original)) {
        const scene = await renderProject(original);
        if(scene?.renderer!=="blender"){setVisualNote("The layout reference could not render. Your plan is preserved; retry when Blender is available.");return;}
      }
      const result = await post<{visual: ProjectSpec["conceptVisual"] | null; scene: ProjectSpec["scene"]; mode: string; reason: string | null}>("/api/visualize", {project: original, photos: brief.photos, demoMode}, 300000);
  ...
  function downloadProject() {
    if (!project) return;
    const blob = new Blob([JSON.stringify({ project, site: siteForStorage(site), disclosure: "Synthetic planning budget and preliminary feasibility; not construction documentation." }, null, 2)], { type: "application/json" });
  ...
  function downloadBidPackage() {
    if (!project) return;
    const url = URL.createObjectURL(new Blob([bidPackageMarkdown(project, siteForStorage(site))], { type: "text/markdown" }));
  ...
  const appearancePreview = currentConceptImage(project) || layoutConceptImage(project);
    ...
    <main className="workspace">
      ...
      <section className={`design-workspace ${stage === "concepts" ? "concept-context" : ""}`}>
        <div className="visual-panel">
          <div className={`visual-toolbar ${conceptPreview ? "has-photoreal" : ""}`}><span className="property-label"><MapPin size={13}/>{site.isDemo ? "24 Maple Lane · illustrative property" : brief.propertyAddress}</span>{project && <div className="view-switch"><button className={view === "concept" ? "selected" : ""} onClick={() => { setView("concept"); if (site.isDemo && !matchingScene(project) && !renderBusy) void renderProject(project); }}>{site.isDemo?"Concept":"Photo concept"}</button><button className={view === "render" ? "selected" : ""} onClick={() => { setView("render"); if (!matchingScene(project) && !renderBusy) void renderProject(project); }}>Blender / 3D</button><button className={view === "plan" ? "selected" : ""} onClick={() => setView("plan")}>Site plan</button></div>}</div>
          {stage === "brief" ? <PropertyContext key={site.propertyAddress} brief={brief} site={site}/> : <ProjectVisual project={project} site={site} view={view} renderBusy={renderBusy} propertyPhoto={brief.photos[0]?.dataUrl}/>}
          {project && stage === "design" && site.isDemo && <div className="concept-controls"><div><strong>Your design, in the same place.</strong><p>{renderBusy ? "Rendering the current layout in detail…" : "The AI finish uses this Blender layout as its reference. Check exact positions, sizes and rotations in the site plan."}</p></div><button className="button secondary" disabled={!!busy || renderBusy} onClick={() => void regenerateAppearance()}><Sparkles size={15}/>{renderBusy ? "Rendering…" : conceptPreview ? "Regenerate concept" : "Render concept"}</button></div>}
          ...
          {project && stage === "design" && !site.isDemo && <div className="concept-controls"><div><strong>This property, transformed.</strong><p>{visualNote || "Your photos lead the concept. Available public aerial imagery provides location context; layout dimensions remain preliminary."}</p></div><button className="button secondary" disabled={!!busy} onClick={()=>void regenerateAppearance()}><Sparkles size={15}/>{appearancePreview?"Regenerate photo concept":"Generate photo concept"}</button></div>}
          {project && stage === "design" && site.isDemo && <details className="appearance-study"><summary>Explore an AI appearance study</summary><p>Explore materials and atmosphere. AI may shift objects; the Concept, Blender and Site plan tabs show the exact layout.</p>{appearancePreview && <img src={appearancePreview} alt="Illustrative AI appearance study; feature placement may differ from the current site plan"/>}<button className="button secondary" disabled={!!busy || renderBusy} onClick={() => void regenerateAppearance()}><Sparkles size={15}/>Generate AI appearance study</button>{visualNote && <p role="status">{visualNote}</p>}</details>}
        ...
        <aside className="brief-panel">
          {stage === "brief" ? <><div className="panel-title"><span className="small-number">01</span><h2>Start with your address.</h2></div><form onSubmit={e => { e.preventDefault(); void generate(); }}>
            <label className="field-label" htmlFor="address">PROPERTY ADDRESS</label><div className="input-with-icon"><MapPin size={16}/><input id="address" disabled={!!busy} value={brief.propertyAddress} placeholder="Street, city, state ZIP" autoComplete="street-address" onChange={e => { const address = e.target.value; setBrief({ ...brief, propertyAddress: address, useDemoSite: address === DEMO_ADDRESS }); setSite(siteForAddress(address, address === DEMO_ADDRESS)); }} required maxLength={250}/></div>
            <div className="address-helper"><span>U.S. residential address · property data may be unavailable</span><button type="button" disabled={!!busy} onClick={()=>void lookupProperty()}>Find my property</button><button type="button" disabled={!!busy} onClick={() => { setBrief(canonicalBrief); setSite(canonicalSiteContext); }}>Use demo property</button></div><label className="field-label" htmlFor="brief">WHAT ARE YOU IMAGINING?</label><textarea disabled={!!busy} id="brief" rows={5} value={brief.description} onChange={e => setBrief({ ...brief, description: e.target.value })} minLength={10} maxLength={4000} required/>
            <label className="field-label" htmlFor="budget">YOUR BUDGET</label><div className="budget-input"><span>$</span><input disabled={!!busy} id="budget" type="number" min={5000} max={1000000} step={500} value={brief.budget} onChange={e => setBrief({ ...brief, budget: Number(e.target.value) })} required/><span>USD</span></div>
      ...
      {stage === "concepts" && <section className="concept-section"><div className="section-heading"><h2>Find your kind of outdoors.</h2><span>01 — 03 / CURATED DIRECTIONS</span></div><div className="concept-grid">{displayedConcepts.map((concept, i) => <button className={`concept-card ${concept.recommended ? "recommended" : ""}`} key={concept.id} onClick={() => void selectConcept(concept)} disabled={!!busy}><div className={`concept-swatch ${concept.palette}`}>{site.isDemo?<ConceptThumbnail brief={brief} concept={concept} site={site}/>:brief.photos[0]?<img src={brief.photos[0].dataUrl} alt="Your property photo; choose a scope and customize before generating its transformation"/>:site.propertyContext?.images.find(image=>image.provider==="usgs")?<img className="aerial-context-image" src={site.propertyContext.images.find(image=>image.provider==="usgs")!.imageUrl} alt="Public aerial context around your address, not a generated design"/>:<div className="property-image-empty">Add a property photo to visualize this scope</div>}<small className="reference-label">{site.isDemo?"AI FINISHES · BLENDER LAYOUT":brief.photos.length?"YOUR PHOTO · PROPOSED SCOPE BELOW":"PROPERTY CONTEXT · PROPOSED SCOPE BELOW"}</small><span>0{i + 1}</span><div className="material-chips"><i/><i/><i/></div>{concept.recommended && <b><Sparkles size={11}/>Our pick for you</b>}</div><div className="concept-body"><span className="concept-tier">{concept.palette==="meadow"?"01 · Simple refresh":concept.palette==="terrace"?"02 · Gather & entertain":"03 · Poolside living"}</span><div className="concept-name"><h3>{concept.title}</h3><MoveUpRight size={19}/></div><p>{concept.designDirection}</p><div className="concept-tags">{concept.majorElements.map(e => <span key={e}>{e}</span>)}</div><p className={`concept-budget-note ${concept.budgetRange.low>brief.budget?"over":""}`}>{concept.budgetRange.low>brief.budget?`Above your ${dollars(brief.budget)} budget · explore or reduce scope`:concept.budgetRange.high>brief.budget?`Upper range exceeds your ${dollars(brief.budget)} budget`:"Preliminary installed range"}</p><div className="concept-price"><strong>{dollars(concept.budgetRange.low)}–{dollars(concept.budgetRange.high)}</strong><span>Explore this idea<ArrowUpRight size={14}/></span></div></div></button>)}</div></section>}
      ...
      <details className="provenance"><summary><span><Layers3 size={16}/>What we know about this site</span><span>Sources & assumptions<ChevronDown size={15}/></span></summary><p>{site.summary}</p><div className="fact-grid">{(["rearSetback", "sideSetback", "zoningDistrict", "lotArea", "easements", "historicStatus"] as const).map(key => { const fact = site[key]; return <div key={key}><span>{key.replace(/([A-Z])/g, " $1")}</span><strong>{fact.value === null ? "Not verified" : `${fact.value}${fact.unit ? ` ${fact.unit}` : ""}`}</strong><small>{fact.status} · {fact.source}<br/>{fact.note}</small></div>; })}</div><p className="form-note">Property imagery provides context. Parcel boundaries, site dimensions and municipal rules still require verification.</p></details>
      </>}
      {stage === "editor" && project && <LayoutEditor project={project} site={site} demoMode={demoMode} onAssistantApply={(next, engine) => { setProject(next); setMeta(engine); setQuotes([]); setRecommendation(null); }} onChange={(next, committed) => { setProject(next); setQuotes([]); setRecommendation(null); if (committed) setMeta(null); }} onClose={() => { setStage("design"); setView("concept"); if(!site.isDemo && !currentConceptImage(project)) void regenerateAppearance(project); }}/>}
