...
export function DreamBid() {
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      {stage === "concepts" && <section className="concept-section"><div className="section-heading"><h2>Find your kind of outdoors.</h2><span>01 — 03 / CURATED DIRECTIONS</span></div><div className="concept-grid">{displayedConcepts.map((concept, i) => <button className={`concept-card ${concept.recommended ? "recommended" : ""}`} key={concept.id} onClick={() => void selectConcept(concept)} disabled={!!busy}><div className={`concept-swatch ${concept.palette}`}><img src={`/demo/photoreal/${concept.palette==="meadow"?"meadow-simple":concept.palette==="terrace"?"terrace-kitchen":"retreat-pool"}.png`} alt={`Retained AI concept reference for ${concept.title}; illustrative, not a current-property photo or a freshly generated design`}/><small className="reference-label">AI CONCEPT REFERENCE · FIXTURE</small><span>0{i + 1}</span><div className="material-chips"><i/><i/><i/></div>{concept.recommended && <b><Sparkles size={11}/>Our pick for you</b>}</div><div className="concept-body"><span className="concept-tier">{concept.palette==="meadow"?"01 · Simple refresh":concept.palette==="terrace"?"02 · Gather & entertain":"03 · Poolside living"}</span><div className="concept-name"><h3>{concept.title}</h3><MoveUpRight size={19}/></div><p>{concept.designDirection}</p><div className="concept-tags">{concept.majorElements.map(e => <span key={e}>{e}</span>)}</div><p className={`concept-budget-note ${concept.budgetRange.low>brief.budget?"over":""}`}>{concept.budgetRange.low>brief.budget?`Above your ${dollars(brief.budget)} budget · explore or reduce scope`:concept.budgetRange.high>brief.budget?`Upper range exceeds your ${dollars(brief.budget)} budget`:"Preliminary installed range"}</p><div className="concept-price"><strong>{dollars(concept.budgetRange.low)}–{dollars(concept.budgetRange.high)}</strong><span>Explore this idea<ArrowUpRight size={14}/></span></div></div></button>)}</div></section>}
