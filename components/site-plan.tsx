...
export type PlanInteraction = {
  ...
  onStart: (event: PointerEvent<SVGGElement>, element: ProjectElement, mode: "move" | "resize") => void;
...
export function SitePlan({ project, site, highlighted = [], compact = false, interaction }: { project: ProjectSpec | null; site: SiteContext; highlighted?: string[]; compact?: boolean; interaction?: PlanInteraction }) {
  ...
  return <svg className={`site-plan ${compact ? "compact" : ""} ${interaction ? "interactive-plan" : ""}`} viewBox="0 0 570 620" role={interaction ? "group" : "img"} aria-label={`${site.isDemo ? "Illustrative" : "Assumed"} site plan${interaction ? "; select a feature, drag it or use arrow keys; drag its corner to resize" : " showing project footprints, tree protection and setback envelope"}`} onPointerMove={interaction?.onMove} onPointerUp={interaction?.onEnd} onPointerCancel={interaction?.onCancel}>
    ...
    {elements.map(e => {
      ...
      return <g key={e.id} role={interaction ? "button" : undefined} tabIndex={interaction ? 0 : undefined} aria-label={interaction ? `${e.label}, ${e.size.widthFt} by ${e.size.depthFt} feet${e.preserved ? ", locked" : ""}${warning ? ", feasibility warning" : ""}` : undefined} aria-pressed={interaction ? selected : undefined} data-element-id={e.id} className={interaction ? `plan-feature ${e.preserved ? "locked" : ""}` : undefined} onPointerDown={event => { if (interaction) { interaction.onSelect(e.id); interaction.onStart(event, e, "move"); } }} onKeyDown={event => interaction?.onKey(event, e)}>
        ...
        {selected && !e.preserved && <g role="button" aria-label={`Resize ${e.label}`} className="resize-handle" onPointerDown={event => { event.stopPropagation(); interaction?.onStart(event, e, "resize"); }}><rect x={x+ew-7} y={y-7} width="14" height="14" rx="3" fill="#254e45" stroke="white" strokeWidth="2"/><path d={`M${x+ew-3} ${y+3} l6 -6 M${x+ew-3} ${y-3} h6 v6`} stroke="white" fill="none" strokeWidth="1"/></g>}
      </g>;
    })}
    ...
    <text x="64" y="607" fontSize="8" fill="#7b866f">{interaction ? "DRAG TO MOVE · CORNER TO RESIZE · ARROW KEYS MOVE 1 FT" : "CONCEPTUAL FOOTPRINTS · SUBJECT TO SURVEY"}</text>
  </svg>;
}
