...
Created an 83-second 1080p H.264/AAC walkthrough of the actual app at `3740226`, using the owner's supplied narration and OpenAI Cedar AI speech. Concept selection is labeled deterministic playback; the layout budget edit and quote assessment used live Astra. Editing removes API waits without presenting fixture results as live. The video shows catalog additions, resizing, a setback conflict, the protected tree, a $39,020 revised project, normalized synthetic bids and an exported package. The transcript, captions and observed figures are in `docs/DEMO.md`; the final video is intentionally retained in `public/demo/dreambid-demo.mp4`, with a clickable preview at the top of the README. Raw recordings and production intermediates remain ignored.
...
Public playback verification caught GitHub attachment URLs that worked with owner authentication but returned 404 for anonymous viewers, including after README publication. Replaced that attachment with the exact final MP4 served as a retained public asset and a linked poster, plus a GitHub download fallback. The final file SHA-256 is `75f8a951e0aac86a9265898e3c86e28d0fb32879a2d42d26ac8346ba2feed773`. No application logic changed. `npm run build` passed again with the retained video asset.
