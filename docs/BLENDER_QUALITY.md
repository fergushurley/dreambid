...
The max preset uses Cycles, up to 64 samples, adaptive sampling at 0.04, denoising, 1920×1280 output, 12 light bounces and CPU rendering for predictable startup on the demo Mac. Older 128-sample property-only scenes rendered quickly, but the fuller garden surroundings need a 64-sample preset to stay within the five-minute runtime limit. The retained simple/kitchen scenes were completed at 96 samples; subsequent live renders use 64 samples with denoising. A five-minute timeout preserves a usable fallback. The UI identifies the render's actual source and quality; a failed render is never presented as successful Blender output.
